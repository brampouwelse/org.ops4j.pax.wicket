 ...
 */
package org.ops4j.pax.wicket.namespace;
...
import org.ops4j.pax.wicket.core.AbstractPageFactory;
...
//                    e.setCallback(new ComponentProxy(injectionSource, overwrites));
                    return /*(WebPage)*/ pageClass.getConstructor(PageParameters.class).newInstance(new Object[]{ params });
                } catch (Exception e) {
...
//                e.setSuperclass(pageClass);
//                e.setCallback(new ComponentProxy(injectionSource, overwrites));
                return /*(WebPage)*/ pageClass.newInstance();
            } catch (Exception e) {
                throw new RuntimeException(String.format("Creation of %s not possible", pageClass.getName()), e);
            }
        }
